...
const WordFrequencyChart = ({ data, loading, error, onSelectArticleIds, onRefresh }) => {
  ...
  if (error) {
    return (
      <Box p={2}>
        ...
        >
          Error loading word frequency data: {error}
        </Alert>
      </Box>
    );
  }
          ...
          {barChartData.length > 0 ? (
            <ResponsiveBar
              ...
              onClick={(node) => {
                ...
                onSelectArticleIds(articleIds, `Word: ${node.data.word}`);
              }}
              borderRadius={4}
