...
def get_text(msg):
    try:   
        ...
        if msg.is_multipart():
            for part in msg.walk():
                try:
                    ...
                    logging.error(f"Error decoding message part: {e}")
                    pass
            return full_message
        else:
            try:
        ...
        logging.error(f"Error getting message text: {e}")
        pass
...
def get_attachments(msg):
    ...
    for attachment in msg.iter_attachments():
        ...
        file["md5"] = hashlib.md5(attachment.get_payload(decode=True)).hexdigest()
        file["sha1"] = hashlib.sha1(attachment.get_payload(decode=True)).hexdigest()
        file["sha256"]=hashlib.sha256(attachment.get_payload(decode=True)).hexdigest()
...
def get_hops(msg):
    ...
    for hop in hops:
        ...
        if "from" in hop and "by" in hop:
            ...
            hops_parsed.append({'number': hop_number, 'from': from_value, 'by': by_value, 'with': with_value, 'date': date_value})
        else:
            logging.warning(f"hop does not contain all the required substrings: {hop}")
    return hops_parsed
