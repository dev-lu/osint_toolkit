...
from modules.newsfeed.agent import (analyze_and_rank_top_articles, analyze_top_articles_stream)
...
async def get_news():
    ...
    return newsfeed.get_news_from_db()
...
async def get_paginated_articles_route(
    ...
    articles_data = await asyncio.to_thread(
        newsfeed.fetch_paginated_articles,
...
async def get_recent_articles(
    ...
    recent_articles = await asyncio.to_thread(crud.get_recent_news_articles, db, time_filter)
...
async def fetch_news(
    ...
    background_tasks.add_task(newsfeed.fetch_and_store_news, db)
...
async def fetch_and_get_news(db: Session = Depends(get_db)):
    ...
    return newsfeed.get_news_from_db(db)
