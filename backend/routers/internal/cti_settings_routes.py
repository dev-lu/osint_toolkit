...
@router.get("/api/settings/cti", response_model=schemas.CTISettingsSchema, tags=["OSINT Toolkit modules"])
def get_cti_settings(db: Session = Depends(get_db)):
    cti_settings = crud.get_cti_settings(db)
    return cti_settings.to_dict()

@router.put("/api/settings/cti", response_model=schemas.CTISettingsSchema, tags=["OSINT Toolkit modules"])
