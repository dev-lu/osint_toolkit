...
from .utils.fetching import parse_feed, fetch_article_full_text, extract_and_categorize_iocs
...
async def check_article_exists(db: Session, link: str) -> bool:
    ...
    try:
        return crud.newsfeed_crud.news_article_exists(db=db, link=link)
...
async def store_article(
    ...
    try:
        ...
        crud.newsfeed_crud.create_news_article(db=db, news_article=news_article)
...
async def fetch_and_store_news(db: Session) -> None:
    ...
    try:
        config = crud.newsfeed_crud.get_newsfeed_config(db=db)
        keyword_matching_enabled = config.keyword_matching_enabled
        keywords = [k.keyword for k in crud.get_keywords(db=db)] if keyword_matching_enabled else []

        feeds = [feed.to_dict() for feed in crud.get_newsfeed_settings(db=db) if feed.enabled]
...
def fetch_paginated_articles(
    ...
    try:
        # Base query
        articles_query = db.query(models.newsfeed_model.NewsArticle)

        if start_date:
            articles_query = articles_query.filter(
                models.newsfeed_model.NewsArticle.date >= start_date
            )
        if end_date:
            articles_query = articles_query.filter(
                models.newsfeed_model.NewsArticle.date <= end_date
            )
        ...
        for field, is_null in null_filters.items():
            if is_null is not None:
                column = getattr(models.newsfeed_model.NewsArticle, field)
                articles_query = articles_query.filter(
        ...
        if tlp:
            articles_query = articles_query.filter(
                models.newsfeed_model.NewsArticle.tlp == tlp
            )
        if read is not None:
            articles_query = articles_query.filter(
                models.newsfeed_model.NewsArticle.read == read
            )
        ...
        # Apply pagination
        articles = (
            articles_query.order_by(models.newsfeed_model.NewsArticle.date.desc())
...
async def get_news_from_db(db: Session) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        news_articles = (
            ...
            .filter(models.NewsArticle.date >= cutoff_date)
            .order_by(models.NewsArticle.date.desc())
            .all()
        )
